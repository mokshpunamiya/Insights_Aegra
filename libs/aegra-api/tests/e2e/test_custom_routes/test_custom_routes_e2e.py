...
def test_openapi_includes_custom_routes():
    ...
    # These routes exist in eerly_studio
    assert "/register" in paths, "Register route should be in OpenAPI spec"
    assert "/login" in paths, "Login route should be in OpenAPI spec"
    assert "/refresh-token" in paths, "Refresh token route should be in OpenAPI spec"

    elog(
        "OpenAPI spec verification",
        {
            "auth_routes_found": [path for path in paths if path in ["/register", "/login", "/refresh-token"]],
